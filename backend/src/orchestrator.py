...
class DebateOrchestrator:
    ...
    def run_stream(self, cancel_check: Callable[[], bool] | None = None) -> Generator[dict, None, None]:
        ...
        yield 的事件格式:
            ...
            {"type": "error", "message": "...", "agent": "..."}
            {"type": "cancelled"}

        Args:
            cancel_check: 可选的回调，每轮/每 Agent 前调用；
                          返回 True 时辩论中止，yield cancelled 事件。
        """
        for round_num in range(1, self.total_rounds + 1):
            if cancel_check and cancel_check():
                yield {"type": "cancelled", "message": "辩论已被取消"}
                return
            ...
            for agent in self.agents:
                ...
                try:
                    ...
                    error_msg = f"{agent.name} 发言失败: {str(e)}"
                    yield {"type": "error", "message": error_msg, "agent": agent.name, "round": round_num}
                    # 用错误消息作为占位发言，让辩论可以继续
                    fallback = f"[{agent.name} 因 API 错误未能发言: {str(e)}]"
                    msg = DebateMessage(agent_name=agent.name, content=fallback, round_num=round_num)
                    self.record.add_message(msg)
