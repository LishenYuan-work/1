...
const sampleMaterials = [
  { id: "knowledge-base", title: "企业知识库升级", category: "技术方案", topic: "评估企业知识库升级方案的收益、风险与实施约束", summary: "适合演示检索增强、权限治理与迁移成本的交叉评审。", tags: ["知识库", "RAG", "迁移"] },
  { id: "customer-service", title: "客服自动化项目", category: "立项材料", topic: "评估客服自动化项目的投入产出、服务质量风险与上线条件", summary: "覆盖人机协同、合规审查和服务指标设计。", tags: ["客服", "自动化", "合规"] },
  { id: "data-platform", title: "数据平台整合", category: "调研主题", topic: "评估多业务线数据平台整合的收益、技术风险与组织约束", summary: "适合测试多来源证据、数据治理和组织协作问题。", tags: ["数据治理", "平台", "组织"] },
];
  ...
  return <div className="review-shell">
    ...
    <main className="review-main">
      ...
      {view === "review" && <div className="page-content review-view">
        ...
        <section className="sample-section"><div className="section-heading"><div><span className="section-kicker"><BookOpen size={14} />快速开始</span><h2>示例评审材料</h2></div><span className="muted-copy">选择一个示例即可载入主题</span></div><div className="sample-grid">{sampleMaterials.map((sample) => <article className="sample-card" key={sample.id}><div className="sample-card-top"><span className="sample-icon"><FileText size={17} /></span><span className="sample-category">{sample.category}</span></div><h3>{sample.title}</h3><p>{sample.summary}</p><div className="tag-row">{sample.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><button className="sample-action" onClick={() => loadSample(sample)}>使用此示例 <ExternalLink size={14} /></button></article>)}</div></section>
