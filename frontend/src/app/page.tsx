...
function MarkdownReport({ markdown }: { markdown: string }) {
  ...
  return <div className="report report-preview report-markdown" aria-label="评审报告内容">{lines.map((line, index) => {
    const value = line.trim();
    if (!value) return <div className="report-spacer" key={`spacer-${index}`} />;
    if (value.startsWith("## ")) return <h3 key={index}>{value.slice(3)}</h3>;
    if (value.startsWith("- ")) return <div className="report-bullet" key={index}><span>•</span><span>{renderReportInline(value.slice(2), index)}</span></div>;
    return <p key={index}>{renderReportInline(value, index)}</p>;
  })}</div>;
}

function renderReportInline(value: string, lineIndex: number): ReactNode {
  const parts = value.split(/(\[[^\]]+\]\(https?:\/\/[^)\s]+\))/g);
  return parts.map((part, index) => {
    const match = part.match(/^\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)$/);
    if (!match) return <span key={`${lineIndex}-${index}`}>{part}</span>;
    return <a key={`${lineIndex}-${index}`} href={match[2]} target="_blank" rel="noreferrer">{match[1]} <ExternalLink size={11} /></a>;
  });
}
