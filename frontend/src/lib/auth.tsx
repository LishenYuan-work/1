...
type AuthState = { user: UserProfile | null; loading: boolean; login: (email: string, password: string) => Promise<void>; register: (data: Parameters<typeof api.auth.register>[0]) => Promise<void>; guestLogin: () => void; logout: () => void; };
...
const Context = createContext<AuthState>({ user: null, loading: true, login: async () => {}, register: async () => {}, guestLogin: () => {}, logout: () => {} });
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  function guestLogin() { setUser(guestProfile); }
  function logout() { if (!user?.is_guest) { void supabase?.auth.signOut(); api.auth.logout().catch(() => undefined); } setUser(null); }
